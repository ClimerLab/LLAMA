...
void Graph::readGML(const std::string &file_name) {
	...
	while (true)
	{
		...
		if (source_found && target_found && (strncmp(tmp_str, "]", 1) == 0))
		{
...
bool Graph::addEdge(const std::size_t source_id, const std::size_t target_id, const double weight)
{
...
bool Graph::removeEdge(const std::size_t source_id, const std::size_t target_id)
{
	assert(source_id < vertices.size());
	assert(target_id < vertices.size());
...
std::vector<std::size_t> Graph::getMinNeighbors(const std::size_t source_id, const std::size_t target_id)
{
...
std::vector<std::size_t> Graph::getMinSepFromNeighbors(const std::size_t source_id, const std::size_t target_id)
{
	std::size_t u, v;
...
std::size_t Graph::seperateGraphIntoComponents(const std::string &out_dir, const std::string &output_tag) 
{
	Vertex *cur_vert;
